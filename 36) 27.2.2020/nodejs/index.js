...
//enable session
app.use(expresssession({ secret: "riji5953tj45", cookie: { maxAge: 1000 * 60 * 10 } }));
...
app.get("/courses", function(req, res) {
  ...
  CourseModel.find(function(err, data) {
    if (err == null)
    {
      ...
      res.render("courses.hbs", { courses: data, currentCourse: req.session.currentCourseName, layout: false } );
      //res.send(data);
    }
    else
    {
...
app.post("/insert-course", function(req, res) {
  ...
  newCourse.save(function(err)
  {
    if (err == null)
    {
      ...
      req.session.currentCourseName = req.body.coursename;
